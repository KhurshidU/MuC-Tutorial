	    ...
	    streamlog_out( DEBUG1 ) << "Creating " << *iter << " hits" << std::endl;
            ...
            for (int i = 0; i < nElements; ++i)
            {
                try
                {
                    ...
                    m_calorimeterHitVector_prsk.push_back(pCaloHit);
...
/// Inserting SoftKiller


std::vector<fastjet::PseudoJet> kh_soft_killer{
// convert to pseudojet list
  PseudoJetList m_calorimeterHitVector_sk = _fju->convertFromRecParticle(m_calorimeterHitVector_prsk);
  ...
  std::cout << "# of pfos in PseudoJet before soft killer   " << m_calorimeterHitVector_postsk.size()<<std::endl;
  for (unsigned int i=0; i< m_calorimeterHitVector_postsk.size(); i++){
        const fastjet::PseudoJet &jet = m_calorimeterHitVector_postsk[i];
        std::cout << "pt = " << jet.pt()
	    << ", rap = " << jet.rap()
        << ", phi = " <<jet.phi()
	    << ", mass = " << jet.m() << std::endl;
  ...
  double pt_threshold;
  std::vector<fastjet::PseudoJet>  m_calorimeterHitVector_postsk;
  soft_killer.apply(m_calorimeterHitVector_sk,  m_calorimeterHitVector_postsk, pt_threshold);
  return m_calorimeterHitVector_postsk;
}

m_calorimeterHitVector.insert(m_calorimeterHitVector.end(), m_calorimeterHitVector_postsk.begin(), m_calorimeterHitVector_postsk.end());
